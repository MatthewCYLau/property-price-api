...
public interface IOrdersService
{
    ...
    IEnumerable<OrderAnalysisResponse> GetQuantityByProduct();
}
...
public class OrdersService : IOrdersService
{
    ...
    public IEnumerable<OrderAnalysisResponse> GetQuantityByProduct()
    {
        var orders = GetOrders();
        var res = orders.GroupBy(order => order.ProductId)
        .Select(g => new OrderAnalysisResponse()
        {
            ProductId = g.Key,
            TotalQuantity = g.Sum(r => r.Quantity),
            AverageQuantity = Math.Round(g.Average(r => r.Quantity), 2),
        });
        return res;
    }
}